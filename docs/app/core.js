...
const jQueryResources = new Map([
  ['1', { version: '1.12.4', integrity: 'sha256-ZosEbRLbNQzLpnKIkEdrPv7lOy9C27hHQ+Xp8a4MxAQ=' }],
  ['2', { version: '2.2.4', integrity: 'sha256-BbhdlvQf/xTY9gja0Dq3HiwQF8LaCRTXxZKRutelT44=' }],
  ['3', { version: '3.6.0', integrity: 'sha256-/xUj+3OJU5yExlq6GSYGSHk7tPXikynS7ogEvDej/m4=' }]
]);
...
const loadJQuery = (ver) => {
  ...
  return fetch(
    `https://code.jquery.com/jquery-${jq.version}.min.js`,
...
const start = () => {
  ...
  const jQueryLoaded = currentJQueryMajorVersion === ver ?
    Promise.resolve() : loadJQuery(ver);

  consoleLog(`== Sample ${sample.value} ==`);
  reset();

  jQueryLoaded.then(() => fetch(`app/${sample.value}.js`))
    .then(res => res.text())
    .then(script => {
